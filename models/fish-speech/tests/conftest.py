...
def variants(request, weights_path):
    config = ModelConfig.from_package("ttsdb_fish_speech")
    ...
    for variant in all_variants:
        ...
        if (variant_dir / "config.json").is_file() and (variant_dir / "model.pth").is_file():
            supported.append(variant)

    if not supported:
        pytest.skip("No supported Fish Speech variants with modern checkpoints found")
